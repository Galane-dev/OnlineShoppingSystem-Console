...
public class CartService : ICartService
{
    ...
    public CartService(DataStore store)
    {
        _store = store;
    }
    ...
    public void AddToCart(Customer customer, int productId, int quantity)
    {
        ...
        // Ensure cart quantity + new quantity won't exceed available stock
        var existingCartItem = customer.Cart.Items.FirstOrDefault(i => i.ProductId == productId);
    ...
    {
        var removed = customer.Cart.RemoveItem(productId);
        if (!removed)
    ...
    public void UpdateQuantity(Customer customer, int productId, int quantity)
    {
        ...
        var product = _store.Products.First(p => p.Id == productId);
