...
public class ReportService : IReportService
{
    ...
    public void GenerateSalesReport()
    {
        ...
        var completedOrders = _store.Orders
            .Where(o => o.Status != OrderStatus.Cancelled)
            .ToList();

        var totalRevenue = completedOrders.Sum(o => o.TotalAmount);
        var ordersByStatus = _store.Orders
            .GroupBy(o => o.Status)
            .Select(g => new { Status = g.Key, Count = g.Count(), Revenue = g.Sum(o => o.TotalAmount) })
            .OrderBy(g => g.Status)
            .ToList();

        Console.WriteLine($"Total Orders    : {_store.Orders.Count}");
        Console.WriteLine($"Total Revenue   : R{totalRevenue:F2}");
        Console.WriteLine($"Average Order   : R{(completedOrders.Any() ? completedOrders.Average(o => o.TotalAmount) : 0):F2}");
        Console.WriteLine();
